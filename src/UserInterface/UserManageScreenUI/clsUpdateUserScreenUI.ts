...
export default class clsUpdateUserScreenUI  extends clsBaseScreenUI
{
    ...
    private static async _PrintInfoUser(user:clsUser):Promise<void>
    {
      ...
      console.log(" User Info : \n")  ;
       ...
       console.log(` Permission : ${user.getPermission}  \n`)  ;
          ...
          public static async ShowUpdateUserScreen():Promise<void>
            {
                  

                  await  clsBaseScreenUI._DrawScreenHeader(" Update User Screen ")  ;


                    let userName  :string  = clsInputValidate.ReadString(" Enter your userName   \n");

                    let oldUser :clsUser =  await clsUser._FindUserByUserName(userName)  ;

                       await this._PrintInfoUser(oldUser)  ;

                       let answer  :string  = clsInputValidate.ReadString(" Do you want to update this user yes or no    \n");

                          if (answer.toLowerCase() == "yes")
                          {
                            let newUser :clsUser  = await this._ReadInfoUser()  ;
    
                      

                               await clsUser._updateUserFromFile(newUser,oldUser)
    
                               await this._PrintInfoUser(newUser)  ;
